...
let from_list size lst = List.fold_left add (create size) lst

(* Test: Associativity *)
let test_associativity =
  Test.make ~name:"Associativity of merging two sets"
    (quad (list int) (list int) (list int) small_nat)
    (fun (lst1, lst2, lst3, size) ->
      let set1 = from_list size lst1 in
      let set2 = from_list size lst2 in
      let set3 = from_list size lst3 in
      merge (merge set1 set2) set3 = merge set1 (merge set2 set3))

(* Test: Neutral Element *)
let test_neutral_element =
  Test.make ~name:"Neutral element for merging"
    (pair (list int) small_nat)
    (fun (lst, size) ->
      let set = from_list size lst in
      let empty_set = create size in
      merge set empty_set = set)

(* Test: Insertion *)
let test_insertion =
  Test.make ~name:"Insertion adds an element"
    (triple (list int) int small_nat)
    (fun (lst, x, size) ->
      let set = from_list size lst in
...
let test_merge =
  Test.make ~name:"Merge contains elements from both sets"
    (triple (list int) (list int) small_nat)
    (fun (lst1, lst2, size) ->
      let set1 = from_list size lst1 in
      let set2 = from_list size lst2 in
...
(* Test: Filter *)
let test_filter =
  Test.make ~name:"Filter retains elements satisfying predicate"
    (pair (list int) small_nat)
    (fun (lst, size) ->
      let set = from_list size lst in
...
(* Test: Map *)
let test_map =
  Test.make ~name:"Map applies a function to all elements"
    (pair (list int) small_nat)
    (fun (lst, size) ->
      let set = from_list size lst in
      let f x = x * 2 in
...
(* Test: Fold_left *)
let test_fold_left =
  Test.make ~name:"Fold_left computes correctly"
    (pair (list int) small_nat)
    (fun (lst, size) ->
      let set = from_list size lst in
...
(* Test: Fold_right *)
let test_fold_right =
  Test.make ~name:"Fold_right computes correctly"
    (pair (list int) small_nat)
    (fun (lst, size) ->
      let set = from_list size lst in
